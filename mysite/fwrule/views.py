   ...
   for f in fwrules:
       ...
       if Network.objects.filter(network_name=f.source.split("-")[0]):
           ...
           if "network" in f.source:
               dict_f['source'] = "%%{%s_network}" %(network.network_interface)
           elif "gateway" in f.source:
               dict_f['source'] = "%%{%s_ipaddress}" %(network.network_interface)
       ...
       if Network.objects.filter(network_name=f.destination.split("-")[0]):
           ...
           if "network" in f.destination:
               dict_f['destination'] = "%%{%s_network}" %(network.network_interface)
           elif "gateway" in f.destination:
               dict_f['destination'] = "%%{%s_ipaddress}" %(network.network_interface)
